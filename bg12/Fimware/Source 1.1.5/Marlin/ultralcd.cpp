...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_cooldown() {
    ...
    void _lcd_ubl_adjust_height_cmd() {
      ...
      sprintf_P(&UBL_LCD_GCODE[ind], PSTR(".%i"), PETG(ubl_height_amount));
      lcd_enqueue_command(UBL_LCD_GCODE);
    }
    ...
    void _lcd_ubl_output_map_lcd() {
      ...
      if (encoderPosition) {
        ...
        if (PETG(step_scaler) >= ENCODER_STEPS_PER_MENU_ITEM) step_scaler = 0;
        encoderPosition = 0;
        lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
      }
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)
    ...
    next_lcd_update_ms = ms + LCD_UPDATE_INTERVAL;
      ...
      const bool encoderPastThreshold = (PETG(encoderDiff) >= ENCODER_PULSES_PER_STEP);
        ...
        if (encoderPastThreshold) {
          int32_t encoderMultiplier = 1;
            ...
            if (encoderRateMultiplierEnabled) {
              int32_t encoderMovementSteps = PETG(encoderDiff) / ENCODER_PULSES_PER_STEP;

              if (lastEncoderMovementMillis) {
                // Note that the rate is always calculated between two passes through the
                // loop and that the PETG of the encoderDiff value is tracked.
