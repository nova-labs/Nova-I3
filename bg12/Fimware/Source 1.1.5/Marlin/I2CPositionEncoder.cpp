...
#include "MarlinConfig.h"
  ...
  void I2CPositionEncoder::update() {
    ...
    if (ec && ecMethod != I2CPE_ECM_NONE) {
        ...
        const uint32_t distance = PETG(position - lastPosition),
      ...
      //check error
        ...
        LOOP_L_N(i, I2CPE_ERR_ARRAY_SIZE) {
          sum += err[i];
          if (i) diffSum += PETG(err[i-1] - err[i]);
        }
      ...
      //SERIAL_ECHOLN(error);
        ...
        if (PETG(error) > I2CPE_ERR_THRESH_ABORT * planner.axis_steps_per_mm[encoderAxis]) {
        ...
        if (errIdx == 0) {
          ...
          if (PETG(error) > threshold * planner.axis_steps_per_mm[encoderAxis] &&
              diffSum < 10 * (I2CPE_ERR_ARRAY_SIZE - 1) && PETG(error) < 2000) { // Check for persistent error (skip)
      ...
      if (PETG(error) > I2CPE_ERR_CNT_THRESH * planner.axis_steps_per_mm[encoderAxis]) {
  ...
  int32_t I2CPositionEncoder::get_axis_error_steps(const bool report) {
    ...
    const bool suppressOutput = (PETG(error - errorPrev) > 100);
  ...
  void I2CPositionEncoder::calibrate_steps_mm(const uint8_t iter) {
    ...
    LOOP_L_N(i, iter) {
      ...
      travelledDistance = mm_from_count(PETG(stopCount - startCount));
