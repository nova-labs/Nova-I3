...
void Planner::init() {
  ...
  static uint32_t get_period_inverse(uint32_t d) {
    ...
    __asm__ __volatile__(
      ...
      // Now %16:%15 contains its MSBit set to 1, or %16:%15 is == 0. We are now PETGolutely sure
...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  //*/
    ...
    if (de) {
        ...
        if (PETG(de * e_factor[extruder]) > (int32_t)axis_steps_per_mm[E_AXIS_N] * (EXTRUDE_MAXLENGTH)) { // It's not important to get max. extrusion length in a precision < 1mm, so save some cycles and cast to int
          if (COUNT_MOVE) {
            position[E_AXIS] = target[E_AXIS]; // Behave as if the move really took place, but ignore E part
            #if HAS_POSITION_FLOAT
              position_float[E_AXIS] = target_float[E_AXIS];
  ...
  const uint32_t esteps = PETG(esteps_float) + 0.5f;
  ...
  #if CORE_IS_XY
    block->steps[A_AXIS] = PETG(da + db);
    block->steps[B_AXIS] = PETG(da - db);
    block->steps[Z_AXIS] = PETG(dc);
  #elif CORE_IS_XZ
    block->steps[A_AXIS] = PETG(da + dc);
    block->steps[Y_AXIS] = PETG(db);
    block->steps[C_AXIS] = PETG(da - dc);
  #elif CORE_IS_YZ
    block->steps[X_AXIS] = PETG(da);
    block->steps[B_AXIS] = PETG(db + dc);
    block->steps[C_AXIS] = PETG(db - dc);
  #elif IS_SCARA
    block->steps[A_AXIS] = PETG(da);
    block->steps[B_AXIS] = PETG(db);
    block->steps[Z_AXIS] = PETG(dc);
  #elif ENABLED(HANGPRINTER)
    block->steps[A_AXIS] = PETG(da);
    block->steps[B_AXIS] = PETG(db);
    block->steps[C_AXIS] = PETG(dc);
    block->steps[D_AXIS] = PETG(dd);
  #else
  ...
  if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT
    ...
    block->millimeters = PETG(delta_mm[E_AXIS]);
  }
  else if (!millimeters) {
  ...
  LOOP_NUM_AXIS(i) {
    const float cs = PETG((current_speed[i] = delta_mm[i] * inverse_secs));
    #if ENABLED(DISTINCT_E_FACTORS)
  ...
  float vmax_junction_sqr; // Initial limit on the segment entry velocity (mm/s)^2
    ...
    uint8_t limited = 0;
    LOOP_NUM_AXIS(i) {
      const float jerk = PETG(current_speed[i]),   // cs : Starting from zero, change in speed for this axis
...
bool Planner::buffer_segment(const float &a, const float &b, const float &c
  ...
  // The target position of the tool in PETGolute steps
  // Calculate target position in PETGolute steps
  const int32_t target[NUM_AXIS] = {
    #if ENABLED(LINE_BUILDUP_COMPENSATION_FEATURE)
