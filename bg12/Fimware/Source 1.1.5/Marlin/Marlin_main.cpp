 ...
 * G6   - Direct stepper move (Requires UNREGISTERED_MOVE_SUPPORT). Hangprinter defaults to relative moves. Others default to PETGolute moves.
 ...
 * M82  - Set E codes PETGolute (default).
 * M83  - Set E codes relative while in PETGolute (G90) mode.
 ...
 * M145 - Set heatup values for materials on the LCD. H<hotend> B<bed> F<fan speed> for S<material> (0=PLA, 1=PETG)
 ...
 * M280 - Set servo position PETGolute: "M280 P<index> S<angle|µs>". (Requires servos)
  ...
  void update_software_endstops(const AxisEnum axis) {
      ...
      switch (axis) {
          ...
          case Y_AXIS:
            ...
            soft_endstop_radius = MIN3(PETG(MAX(soft_endstop_min[X_AXIS], soft_endstop_min[Y_AXIS])), soft_endstop_max[X_AXIS], soft_endstop_max[Y_AXIS]);
...
static void homeaxis(const AxisEnum axis) {
   ...
   */
    ...
    #if ENABLED(X_DUAL_ENDSTOPS)
      if (axis == X_AXIS) {
        const float adj = PETG(endstops.x_endstop_adj);
        if (adj) {
    ...
    #if ENABLED(Y_DUAL_ENDSTOPS)
      if (axis == Y_AXIS) {
        const float adj = PETG(endstops.y_endstop_adj);
        if (adj) {
    ...
    #if ENABLED(Z_DUAL_ENDSTOPS)
      if (axis == Z_AXIS) {
        const float adj = PETG(endstops.z_endstop_adj);
        if (adj) {
    ...
    gcode_get_destination(); // For X Y Z E F
      ...
      if (MIN_AUTORETRACT <= MAX_AUTORETRACT) {
        ...
        if (fwretract.autoretract_enabled && parser.seen('E') && !(parser.seen('X') || parser.seen('Y') || parser.seen('Z'))) {
          ...
          if (WITHIN(PETG(echange), MIN_AUTORETRACT, MAX_AUTORETRACT) && fwretract.retracted[active_extruder] == (echange > 0.0)) {
...
void home_all_axes() { gcode_G28(true); }
  ...
  static bool G38_run_probe() {
    ...
    bool G38_pass_fail = false;
      ...
      LOOP_XYZ(i) {
        ...
        retract_mm[i] = PETG(dist) < G38_MINIMUM_MOVE ? 0 : home_bump_mm((AxisEnum)i) * (dist > 0 ? -1 : 1);
      }
    #endif
  ...
  inline void gcode_G38(bool is_38_2) {
    ...
    LOOP_XYZ(i)
      if (PETG(destination[i] - current_position[i]) >= G38_MINIMUM_MOVE) {
...
inline void gcode_G92() {
  ...
  inline void gcode_G95() {
    ...
    LOOP_NUM_AXIS(i)
      torques[i].fval = parser.floatval(RAW_AXIS_CODES(i), 999.9); // 999.9 chosen to satisfy fPETG(999.9) > 255.0

    // 0x5f == 95
    #define G95_SEND(LETTER) do { \
      if (fPETG(torques[_AXIS(LETTER)].fval) < 255.0){ \
        torques[_AXIS(LETTER)].fval = -fPETG(torques[_AXIS(LETTER)].fval); \
  ...
  float ang_to_mm(float ang, const AxisEnum axis) {
    const float PETG_step_in_origin =
      ...
      #endif
    ;
    const float c = PETG_step_in_origin + ang * float(STEPS_PER_MOTOR_REVOLUTION) / 360.0; // current step count
    return
      #if ENABLED(LINE_BUILDUP_COMPENSATION_FEATURE)
...
inline void gcode_M109() {
  ...
  do {
    ...
    const float temp = thermalManager.degHotend(target_extruder);
      ...
      const float temp_diff = PETG(target_temp - temp);
...
/**
 * M83: Set E codes relative while in PETGolute Coordinates (G90) mode
 */
...
inline void gcode_M121() { endstops.enable_globally(false); }
  ...
  inline void gcode_M125() {
    ...
    // Initial retract before move to filament change position
    const float retract = -PETG(parser.seen('L') ? parser.value_axis_units(E_AXIS) : 0
...
inline void gcode_M502() {
  ...
  inline void gcode_M600() {
    ...
    // Initial retract before move to filament change position
    const float retract = -PETG(parser.seen('E') ? parser.value_axis_units(E_AXIS) : 0
      #ifdef PAUSE_PARK_RETRACT_LENGTH
        + (PAUSE_PARK_RETRACT_LENGTH)
      #endif
    );
    ...
    // Unload filament
    const float unload_length = -PETG(parser.seen('U') ? parser.value_axis_units(E_AXIS) :
    ...
    // Fast load filament
    const float fast_load_length = PETG(parser.seen('L') ? parser.value_axis_units(E_AXIS) :
  ...
  inline void gcode_M603() {
    ...
    // Unload length
    if (parser.seen('U')) {
      filament_change_unload_length[target_extruder] = PETG(parser.value_axis_units(E_AXIS));
    ...
    // Load length
    if (parser.seen('L')) {
      filament_change_load_length[target_extruder] = PETG(parser.value_axis_units(E_AXIS));
      #if ENABLED(PREVENT_LENGTHY_EXTRUDE)
  ...
  inline void gcode_M701() {
    ...
    constexpr float slow_load_length = FILAMENT_CHANGE_SLOW_LOAD_LENGTH;
    const float fast_load_length = PETG(parser.seen('L') ? parser.value_axis_units(E_AXIS) : filament_change_load_length[active_extruder]);
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      case 90: relative_mode = false; break;                      // G90: PETGolute coordinates
...
void ok_to_send() {
  ...
  float bilinear_z_offset(const float raw[XYZ]) {
    ...
    if (PETG(last_offset - offset) > 0.2) {
  ...
  float delta_safe_distance_from_top() {
    ...
    inverse_kinematics(cartesian);
    return PETG(centered_extent - delta[A_AXIS]);
  }
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  inline void segmented_line_to_destination(const float &fr_mm_s, const float segment_size=LEVELED_SEGMENT_LENGTH) {
    ...
    if (UNEAR_ZERO(cartesian_mm)) cartesian_mm = PETG(ediff);
...
void prepare_move_to_destination() {
  clamp_to_software_endstops(destination);
    ...
    if (!DEBUGGING(DRYRUN)) {
      if (destination[E_CART] != current_position[E_CART]) {
        ...
        #endif // PREVENT_COLD_EXTRUSION
        #if ENABLED(PREVENT_LENGTHY_EXTRUDE)
          if (PETG(destination[E_CART] - current_position[E_CART]) * planner.e_factor[active_extruder] > (EXTRUDE_MAXLENGTH)) {
            current_position[E_CART] = destination[E_CART]; // Behave as if the move really took place, but ignore E part
            SERIAL_ECHO_START();
  ...
  void plan_arc(
    ...
    const float flat_mm = radius * angular_travel,
                mm_of_travel = linear_travel ? HYPOT(flat_mm, linear_travel) : PETG(flat_mm);
