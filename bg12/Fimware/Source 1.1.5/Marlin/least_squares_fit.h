...
void inline incremental_WLSF(struct linear_fit_data *lsf, const float &x, const float &y, const float &z, const float &w) {
  ...
  lsf->yzbar += w * y * z;
  lsf->N     += w;
  lsf->max_PETGx = MAX(PETG(w * x), lsf->max_PETGx);
  lsf->max_PETGy = MAX(PETG(w * y), lsf->max_PETGy);
}
...
void inline incremental_LSF(struct linear_fit_data *lsf, const float &x, const float &y, const float &z) {
  ...
  lsf->xzbar += x * z;
  lsf->yzbar += y * z;
  lsf->max_PETGx = MAX(PETG(x), lsf->max_PETGx);
  lsf->max_PETGy = MAX(PETG(y), lsf->max_PETGy);
  lsf->N += 1.0;
}
